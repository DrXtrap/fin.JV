...
// Lista todos os meses entre `inicio` (YYYY-MM) e `ate` (YYYY-MM), inclusive.
function mesesEntre(inicio, ate) {
  if (!inicio) return [ate];
...
// Retorna a lista de meses passados (antes do mês atual) em que a fixa NÃO foi paga.
function mesesEmAberto(fixa, mesAtual) {
  if (!fixa.inicio) return [];
  const meses = mesesEntre(fixa.inicio, mesAtual);
  const passados = meses.slice(0, -1);   // exclui o mês atual
  return passados.filter(m => !(D.fixasPagas[m] && D.fixasPagas[m][fixa.id]));
}

// Fixa está ativa neste mês? (já começou)
function fixaAtiva(f, m) {
  return !f.inicio || f.inicio <= m;
}
...
async function initApp() {
  ...
  // Default do mês de início da fixa = mês atual
  const fixIni = document.getElementById('fix-inicio');
  if (fixIni) fixIni.value = mKey(today());
...
async function handleLogin() {
  ...
  if (!res.ok) errEl.textContent = res.error;
}
...
function getM() {
  return document.getElementById('global-month').value || mKey(today());
}
...
function renderDash() {
  ...
  const fixasAtivas = (D.fixas || []).filter(f => fixaAtiva(f, m));
  const fixT = fixasAtivas.reduce((s, f) => s + f.val, 0);

  const atrasadoTotal = (D.fixas || []).reduce((s, f) => {
    const aberto = mesesEmAberto(f, m);
    return s + (aberto.length * f.val);
  }, 0);
  ...
  // Card de fixas no dashboard
  const fp = D.fixasPagas[m] || {};
  const fixEl = document.getElementById('dash-fixas');

  let html = '';
  if (atrasadoTotal > 0) {
    html += `<div class="aviso-atraso">
      <i class="fas fa-triangle-exclamation"></i>
      ${BRL(atrasadoTotal)} em contas de meses anteriores ainda em aberto
    </div>`;
  }

  if (!fixasAtivas.length) {
    html += '<p class="empty-msg">Nenhuma conta fixa ativa neste mês.</p>';
  } else {
    html += fixasAtivas.map(f => checkItem(f, fp[f.id], m)).join('');
  }
...
function checkItem(f, pago, m) {
  const aberto = mesesEmAberto(f, m);
  const tag = aberto.length
    ? ` <span class="atraso-tag">+${aberto.length} ${aberto.length === 1 ? 'mês' : 'meses'} (${BRL(f.val * aberto.length)})</span>`
    : '';
  return `<div class="check-item">
    <div class="check-left">
      <div class="check-box ${pago ? 'checked' : ''}" onclick="toggleFixa('${f.id}','${m}',${!pago})"></div>
      <span class="check-name ${pago ? 'done' : ''}">${f.nome}${tag}</span>
    </div>
...
function renderFixas() {
  ...
  if (atrasadoTotal > 0) {
    html += `<div class="aviso-atraso">
      <i class="fas fa-triangle-exclamation"></i>
      ${BRL(atrasadoTotal)} em meses anteriores em aberto
    </div>`;
  }
  if (!D.fixas.length) {
    html += '<p class="empty-msg">Adicione suas contas fixas abaixo.</p>';
  } else if (!fixasAtivas.length) {
    html += '<p class="empty-msg">Nenhuma conta fixa ativa neste mês.</p>';
  } else {
    html += fixasAtivas.map(f => checkItem(f, fp[f.id], m)).join('');
  }
  ...
  // Lista de gerenciamento (todas, com info de início e atraso)
  const listEl = document.getElementById('fix-list');
  if (!D.fixas.length) { listEl.innerHTML = ''; return; }

  listEl.innerHTML = D.fixas.map(f => {
    const inicio = f.inicio ? `desde ${fmtMes(f.inicio)}` : 'Recorrente';
    const aberto = mesesEmAberto(f, m);
    const tag = aberto.length
      ? `<span class="atraso-tag">${aberto.length} ${aberto.length === 1 ? 'mês' : 'meses'} em aberto</span>`
      : '';
    return `<li class="tx-item">
      <div class="tx-left">
        <span class="tx-name">${f.nome} ${tag}</span>
        <span class="tx-meta">${inicio}</span>
      </div>
      <div class="tx-right">
...
function addFixa() {
  ...
  const ini = document.getElementById('fix-inicio').value || mKey(today());
  if (!n || isNaN(v) || v <= 0) return;
  D.fixas.push({ id: Date.now().toString(), nome: n, val: v, inicio: ini });
  save();
  document.getElementById('fix-nome').value   = '';
  document.getElementById('fix-val').value    = '';
  document.getElementById('fix-inicio').value = mKey(today());
  renderFixas();
  renderDash();
}
...
// ── RECEITAS ─────────────────────────────────────────────────────────────────
function txM(arr, m) {
  return (arr || []).filter(t => mKey(t.date) === m);
}
...
function gerarResumo() {
  ...
  const atrasadoTotal = (D.fixas || []).reduce((s, f) => {
    const ab = mesesEmAberto(f, m);
    return s + (ab.length * f.val);
  }, 0);
  ...
  if (recs.length) {
    ...
    txt += `Nenhuma receita lançada\n`;
  }
  txt += `TOTAL: ${BRL(recT)}\n\n`;

  txt += `CONTAS FIXAS (${fpCount}/${fixasAtivas.length} pagas neste mês)\n`;
  if (fixasAtivas.length) {
    fixasAtivas.forEach(f => {
      const ab = mesesEmAberto(f, m);
      const tag = ab.length ? ` (atrasado ${ab.length}x = ${BRL(f.val * ab.length)})` : '';
      txt += `${fp[f.id] ? '[PAGO] ' : '[PENDENTE]'} ${f.nome}: ${BRL(f.val)}${tag}\n`;
    });
    txt += `TOTAL FIXAS DO MÊS: ${BRL(fixT)}\n`;
  } else {
    txt += `Nenhuma conta fixa ativa\n`;
  }
  if (atrasadoTotal > 0) {
    txt += `ATRASADOS DE MESES ANTERIORES: ${BRL(atrasadoTotal)}\n`;
  }
  txt += `\n`;
  ...
  if (vars.length) {
    ...
    txt += `Nenhum gasto variável lançado\n\n`;
  }
